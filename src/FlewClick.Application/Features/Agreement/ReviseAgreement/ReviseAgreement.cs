...
namespace FlewClick.Application.Features.Agreement.ReviseAgreement;

public record ReviseAgreementDeliverableItem(
    string DeliverableName,
    int Quantity,
    Dictionary<string, object?>? Configuration,
    string? Notes);

public record ReviseAgreementCommand(
    ...
    string? Conditions,
    string? Notes,
    List<ReviseAgreementDeliverableItem> Deliverables) : IRequest<AgreementDto>;

public class ReviseAgreementValidator : AbstractValidator<ReviseAgreementCommand>
{
    public ReviseAgreementValidator()
    {
...
public class ReviseAgreementHandler(
    ...
    : IRequestHandler<ReviseAgreementCommand, AgreementDto>
{
    public async Task<AgreementDto> Handle(ReviseAgreementCommand request, CancellationToken ct)
    {
        ...
        if (booking.Status != BookingStatus.RevisionRequested)
            throw new DomainException(
                $"Cannot revise agreement when booking status is '{booking.Status}'. Expected '{BookingStatus.RevisionRequested}'.");
